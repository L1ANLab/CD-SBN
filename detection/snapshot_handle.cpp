...
{
    // 0. initialization:
    std::chrono::high_resolution_clock::time_point start_timestamp, refine_candidate_set_start_timestamp;
            ...
            while(maximum_heap_H.size() > 0)
            {
                ...
                if (current_node->GetLevel() < MAX_LEVEL)
                {
                    ...
                    if (CheckPruningConditions(current_node))
                    {
                        ...
                        {
                            HeapEntry* now_child_heap_entry = new HeapEntry(
                                child_node,
                                child_node->GetUbScore(query_radius_idx)
                            );
                            maximum_heap_H.emplace_back(now_child_heap_entry);
                ...
                else
                {
                    ...
                    if (CheckPruningConditions(current_node))
                    {
                        if (current_node->GetUserSet().size() > 0)
                        {
                            ...
                            // #pragma omp task
                            // {
                                std::chrono::high_resolution_clock::time_point leaf_node_start_timestamp = Get_Time();
                                ...
                                else
                                {
                                    ...
                                    std::chrono::high_resolution_clock::time_point compute_community_start_timestamp = Get_Time();
                                    std::unique_ptr<InducedGraph> k_r_sigma_bitruss_subgraph(r_hop_subgraph->ComputeKRSigmaBitrussSimple(
